...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # Precompute the encoder output and reuse it for every token gathered from decoder
...
def get_dataset(config):
    print('Loading dataset', 'langs', f'{config["lang_src"]}-{config["lang_tgt"]}')
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        model.train()
        batch_iterator = tqdm(train_dataloader, desc=f'Processing epoch {epoch:02d}', position=0, leave=True, total=len(train_dataloader))
